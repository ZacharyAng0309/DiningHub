...
namespace DiningHub.Controllers
{
    ...
    public class ShoppingCartController : Controller
    {
        ...
        public ShoppingCartController(DiningHubContext context, UserManager<DiningHubUser> userManager, RoleManager<IdentityRole> roleManager, ILogger<ShoppingCartController> logger, IAmazonSQS sqsClient, AWSSettings awsSettings)
        {
            _context = context;
        ...
        public async Task<IActionResult> ConfirmCheckout(CheckoutViewModel model)
        {
            ...
            using (IDbContextTransaction transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    ...
                    _logger.LogInformation("Sending message to SQS with order details: {OrderId}, {UserName}, {TotalAmount}", order.OrderId, order.UserName, order.TotalAmount);
                    ...
                    _logger.LogInformation("Message sent to SQS with MessageId: {MessageId}, SequenceNumber: {SequenceNumber}", sendMessageResponse.MessageId, sendMessageResponse.SequenceNumber);
